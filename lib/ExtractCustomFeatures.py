...
def ExtractCustomFeatures(CUSTOM_DETECTOR, PATH_TO_LOCAL_FEATURES, DATABASE, KEYFRAMES_DIR, keyframes_list):
    ...
    # Create cameras.
    model1, width1, height1, params1 = 4, 752, 480, np.array((458.654,457.296,367.215,248.375,-0.28340811,0.07395907,0.00019359,1.76187114e-05))
    camera_id1 = db.add_camera(model1, width1, height1, params1)

    if CUSTOM_DETECTOR == 'ALIKE':
        for img in kfrms:            
            if img not in existing_images:
                kp, desc, kp_numb = ALIKE(inverted_img_dict[img][:-3]+'png', PATH_TO_LOCAL_FEATURES)
                kp = kp[:,0:2]
        ...
        for img in kfrms:
            if img not in existing_images:
                ...
                for j in range(len(kp)):
                    all_kpts[j, 0], all_kpts[j, 1] = cv2.KeyPoint_convert(kp)[j][0], cv2.KeyPoint_convert(kp)[j][1]
        ...
        for img in kfrms:            
            if img not in existing_images:
                kp, desc, kp_numb = KeyNet(inverted_img_dict[img][:-3]+'png', PATH_TO_LOCAL_FEATURES)
                kp = kp[:,0:2]
